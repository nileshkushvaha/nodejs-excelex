import Link from "next/link";

export const metadata = { title: "Track a shipment · ExcelEx" };

export default function TrackPage() {
  return (
    <main className="mx-auto max-w-xl px-6 py-20">
      <h1 className="text-2xl font-semibold text-fg">Track a shipment</h1>
      <p className="mt-2 text-sm text-muted">
        Enter an AWB number to see its journey. Public tracking needs no account.
      </p>

      <form className="mt-6 flex gap-2">
        <input
          type="text"
          inputMode="numeric"
          placeholder="AWB number"
          aria-label="AWB number"
          className="flex-1 rounded border border-line-strong px-3 py-2 text-sm outline-none focus:border-accent focus:ring-2 focus:ring-accent-soft"
        />
        <button
          type="submit"
          disabled
          className="rounded bg-accent px-4 py-2 text-sm font-medium text-white disabled:opacity-50"
        >
          Track
        </button>
      </form>

      <p className="mt-4 rounded border border-line bg-surface-2 p-3 text-xs text-muted">
        Tracking becomes live with the shipment module in Phase 3. The screen exists now because
        public tracking is one of the few paths that must stay available even when a client is over
        its storage quota.
      </p>

      <p className="mt-6 text-xs">
        <Link href="/" className="text-muted underline hover:text-fg">
          Back to the public site
        </Link>
      </p>
    </main>
  );
}
