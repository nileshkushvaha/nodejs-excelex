...
- Login history and the jobs table grow with use. The retention sweep is a job like any other; every client gets a "Nightly retention sweep" schedule by default (created the first time the scheduler sees the client, never re-created once an administrator deletes it), so nothing grows for ever unless somebody decides it should.
