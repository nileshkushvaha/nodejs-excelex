...
**6b. Exceptions are kept, per client, grouped by cause.** `ExceptionRecorder` writes every 5xx, final job failure and failed dispatch to `exception_events` and upserts an `exception_groups` row keyed by a fingerprint of source + code + exception + route — count, first/last seen, and a status an operator sets (resolve reopens on regression; ignore sticks). The `/system/exceptions` screen reads them with stacks and the same reference the person quoted; the sweep keeps thirty days of events.
...
- Rate limiting followed the same day: a Redis fixed-window limiter (`core/rate-limit/`), a global per-address guard writing the IETF `RateLimit-*` headers, and a sign-in throttle (per address for the deployment, per email under the client's own `loginThrottleEnabled` switch) that records refusals in login history as `THROTTLED`. It fails open when Redis is unreachable and says so in the log. Express is now told how many proxies to trust (`TRUST_PROXY_HOPS`), and the web app forwards `X-Forwarded-For` on its server-side calls, so both login history and per-address limits see the person rather than the proxy.
