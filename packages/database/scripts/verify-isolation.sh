...
check "all 13 client tables have ENABLE + FORCE row level security" "13" \
  "$(pg "$OWNER" -tA -q -c "SELECT count(*) FROM pg_class c JOIN pg_namespace n ON n.oid=c.relnamespace WHERE n.nspname='public' AND c.relname IN ('branches','users','user_branch_memberships','roles','user_roles','role_permissions','user_permissions','password_policies','password_history','security_settings','sessions','invitations','audit_events') AND c.relrowsecurity AND c.relforcerowsecurity;")"
check "all 13 client tables carry the client_isolation policy" "13" \
