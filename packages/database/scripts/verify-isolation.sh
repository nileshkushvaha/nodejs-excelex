...
check "all 55 client tables have ENABLE + FORCE row level security" "55" \
  "$(pg "$OWNER" -tA -q -c "SELECT count(*) FROM pg_class c JOIN pg_namespace n ON n.oid=c.relnamespace WHERE n.nspname='public' AND c.relname IN ('branches','users','user_branch_memberships','roles','user_roles','role_permissions','user_permissions','password_policies','password_history','security_settings','departments','designations','client_settings','product_types','product_groups','products','zones','destinations','service_centres','sales_executives','charges','charge_components','customers','customer_fuel_surcharges','customer_charges','customer_volumetrics','customer_contacts','consignees','shippers','account_groups','lookups','pin_codes','rate_cards','rate_lines','jobs','job_schedules','login_attempts','mail_settings','mail_messages','password_resets','notifications','exception_events','exception_groups','cms_contents','cms_revisions','cms_redirects','cms_terms','cms_content_terms','cms_media','cms_menus','cms_menu_items','cms_site_settings','sessions','invitations','audit_events') AND c.relrowsecurity AND c.relforcerowsecurity;")"
check "all 55 client tables carry the client_isolation policy" "55" \
